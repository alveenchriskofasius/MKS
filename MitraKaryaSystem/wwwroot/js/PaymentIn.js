const PaymentIn = (function () {
  ...
  const setTypeByAmount = (amount, total) => {
    amount = Number(amount||0); total = Number(total||0);
    if(total>0 && amount>=total){ $("input[name='piType'][value='Full']").prop('checked',true); }
    else if(amount>0 && amount<total){ $("input[name='piType'][value='DP']").prop('checked',true); }
  };
